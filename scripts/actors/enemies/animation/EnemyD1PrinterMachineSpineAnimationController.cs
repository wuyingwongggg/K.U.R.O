...
{
	public partial class EnemyD1PrinterMachineSpineAnimationController : EnemySpineAnimationController
	{
		...
		private EnemyD1PrinterMachineAttackController? _attackController;
		...
		private void HandleAttackAnimations()
		{
			...
			if (!string.IsNullOrEmpty(attackName))
			{
				...
				if (attackName.Equals(controller.Missile01AttackName, _comparison))
				{
					PlayOnceIfNeeded("Skill", SkillAnimation, AttackMixDuration);
					return;
				}
			}
		...
		private EnemyD1PrinterMachineAttackController? ResolveAttackController()
		{
			...
			_attackController = GetNodeOrNull<EnemyD1PrinterMachineAttackController>(AttackControllerPath);
			if (_attackController == null)
				_attackController = Enemy.GetNodeOrNull<EnemyD1PrinterMachineAttackController>(AttackControllerPath);
		...
		private bool IsExpectedHitAnimation(EnemyD1PrinterMachineAttackController controller, string animationName)
		{
			...
			return string.IsNullOrEmpty(expectedAnimation) || string.Equals(animationName, expectedAnimation, _comparison);
		}
	}
}
